...
class AdvGAN_Attack:
  ...
  def train_batch(self, batch_x, batch_labels):
    ...
    return loss_D_GAN.item(), loss_G_fake.item(), loss_adv.item(), loss_perturb.item()


  def fit(self, train_dataloader, epochs, ADVGAN_DIR):

    loss_D_epoch, loss_G_fake_epoch, loss_adv_epoch, loss_perturb_epoch = [], [], [], []
    os.makedirs(ADVGAN_DIR + '/' + str(self.config_name), exist_ok=True)
    ...
    for epoch in range(1, epochs+1):
      ...
      for images, labels in tqdm(train_dataloader):
        ...
        loss_D_batch, loss_G_fake_batch, loss_adv_batch, loss_perturb_batch = self.train_batch(images, labels)
        loss_D_sum, loss_G_fake_sum, loss_adv_sum, loss_perturb_sum = loss_D_sum+loss_D_batch, loss_G_fake_sum+loss_G_fake_batch, loss_adv_sum+loss_adv_batch, loss_perturb_sum+loss_perturb_batch
        
      # print average losses after each epoch
      loss_D_avg, loss_G_fake_avg, loss_adv_avg, loss_perturb_avg = loss_D_sum/len(train_dataloader), loss_G_fake_sum/len(train_dataloader), loss_adv_sum/len(train_dataloader), loss_perturb_sum/len(train_dataloader)
      print("epoch %d:\n loss_D: %.3f,\n loss_G_fake: %.3f,\n loss_adv: %.3f,\n loss_perturb: %.3f, \n" % (epoch, loss_D_avg, loss_G_fake_avg, loss_adv_avg, loss_perturb_avg))
      ...
      # save generator
      if epoch%5==0:
          netGen_file_name = ADVGAN_DIR + '/' + str(self.config_name) + '/Gen_epoch_' + str(epoch) + '.pth'
          torch.save(self.netGen.state_dict(), netGen_file_name)

          netDisc_file_name = ADVGAN_DIR + '/' + str(self.config_name) + '/Disc_epoch_' + str(epoch) + '.pth'
          torch.save(self.netDisc.state_dict(), netDisc_file_name)
    
    # save the config of the model
    with open(ADVGAN_DIR + '/' + str(self.config_name) + '/config.json', 'w') as config_file:
        config_file.write(json.dumps(self.config))
    
    return loss_D_epoch, loss_G_fake_epoch, loss_adv_epoch, loss_perturb_epoch
